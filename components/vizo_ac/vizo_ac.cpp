...
namespace esphome::vizo_ac {
namespace {
const char *const TAG = "vizo_ac";
...
void VizoACClimate::control(const climate::ClimateCall &call) {
...
void VizoACClimate::transmit_state() {
  if (this->mode == climate::CLIMATE_MODE_OFF) {
    this->send_power_toggle_();
    return;
  }
...
uint8_t VizoACClimate::temp_byte_(float temp) {
  const auto value = static_cast<uint8_t>(roundf(clamp(temp, 20.0f, 30.0f)));
  return static_cast<uint8_t>(((value - 16) << 4) | 0x02);
}

uint8_t VizoACClimate::fan_byte_(optional<climate::ClimateFanMode> fan) {
...
uint8_t VizoACClimate::seq_for_temp_(float temp) {
  const auto value = static_cast<uint8_t>(roundf(clamp(temp, 20.0f, 30.0f)));
  if (value <= 23)
    return 0x11;
  if (value <= 26)
